...
| FrankenPHP | 1.4 | Выполняет PHP-код и принимает HTTP-запросы |
...
Сервисы окружения:
- `app` — Laravel через FrankenPHP (HTTP)
- `worker` — обработка очередей (`php artisan queue:work`)
- `scheduler` — расписание задач (`php artisan schedule:work`)
- `db` — PostgreSQL
- `redis` — Redis
